...
static int test_adv(const struct shell *shell, size_t argc, char **argv)
{
	adv_start();
	return 0;
}

static int test_scan(const struct shell *shell, size_t argc, char **argv)
{
	scan_start();
	return 0;
}
...
SHELL_CMD_REGISTER(adv, NULL, "Start advertising", test_adv);
SHELL_CMD_REGISTER(scan, NULL, "Start scan", test_scan);
